...
<script>
  $(document).ready(function () {
    ...
    $('.btn-remove').click(function () {
      // $(this) 可以獲取到當前點擊的 移除 按鈕的 jQuery 對象
      // closest() 方法可以獲取到匹配選擇器的第一個祖先元素，在這裏就是當前點擊的 移除 按鈕之上的 <tr> 標籤
      ...
      var id = $(this).closest('tr').data('id');
        ...
        // 用戶點擊 確定 按鈕，willDelete 的值就會是 true，否則爲 false
        if (!willDelete) {
          return;
        }
    ...
    $('.btn-create-order').click(function () {
      // 構建請求參數，將用戶選擇的地址的 id 和備註內容寫入請求參數
      var req = {
      ...
      $('table tr[data-id]').each(function () {
        ...
        // 如果用戶將數量設爲 0 或者不是一個數字，則也跳過
        if ($input.val() == 0 || isNaN($input.val())) {
          return;
        }
        // 把 SKU id 和數量存入請求參數數組中
        req.items.push({
          sku_id: $(this).data('id'),
          amount: $input.val(),
        })

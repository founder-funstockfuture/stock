                  ...
                  <tr>
                    ...
                    @if($index === 0)
                      ...
                      <td rowspan="{{ count($order->items) }}" class="text-center">
                        ...
                        @else
                          未支付<br>
                          請於 {{ $order->created_at->addSeconds(config('app.order_ttl'))->format('m-d H:i') }} 前完成支付<br>
                          否則訂單將自動關閉
                        @endif
                      </td>
                      <td rowspan="{{ count($order->items) }}" class="text-center">
                      <a class="btn btn-primary btn-sm" href="{{ route('orders.show', ['order' => $order->id]) }}">檢視訂單</a>
                      
                      <!-- 評價入口開始 -->
                      @if($order->paid_at)
                      <a class="btn btn-success btn-sm" href="{{ route('orders.review.show', ['order' => $order->id]) }}">
                      {{ $order->reviewed ? '檢視評價' : '評價' }}
                      </a>
                      @endif
                      <!-- 評價入口結束 -->
                      </td>

...
class VideosController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->title('影片名稱')->width(150);
        $grid->excerpt('摘要')->limit(100)->width(150);
        $grid->youtube_url('youtube 連結')->link()->width(150);
        ...
        $grid->filter(function($filter){
            ...
            // 在這裡新增欄位過濾器
            $filter->equal('category_id', '分類')->select(VideoCategory::all()->pluck('name', 'id'));

            $filter->like('title', '影片名稱');
            $filter->like('excerpt', '摘要');
            $filter->like('youtube_url', 'youtube 連結');
            ...
            $filter->equal('is_block','管理者封鎖')->radio([
                ''   => '全部',
                0    => '否',
                1    => '是',
            ]);
    ...
    protected function form()
    {
        ...
        $form->url('youtube_url', 'youtube 連結')->rules('required');
        ...
        $form->saved(function (Form $form) {
            ...
            if($video = Video::find($video_id)){
                $video_replies_count = VideoReply::where('video_id',$video_id)->count();
